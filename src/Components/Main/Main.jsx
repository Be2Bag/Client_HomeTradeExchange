...
const correctImageUrl = (url) => {
  if (url.startsWith('uploads\\')) {
    return url.replace('uploads\\', 'https://server-hometradeexchange.onrender.com/uploads/');
  }
    ...
    const fetchData = async () => {
      try {
        ...
        if (requestData && Object.keys(requestData).length !== 0) {
          ...
          response = await axios.post('https://server-hometradeexchange.onrender.com/filter', requestData);
        } else {
          // requestData ว่าง, ทำ GET request
          response = await axios.get('https://server-hometradeexchange.onrender.com/listings');
        }
